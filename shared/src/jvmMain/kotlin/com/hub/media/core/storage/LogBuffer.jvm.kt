...
/**
 * JVM actual for [LogBuffer] -- see that `expect` class's KDoc for the overflow policy and
 * sequence-numbering contract this implements. Identical (deliberately) to the Android actual:
 * both targets are JVM-flavored, so the same `synchronized`-guarded `ArrayDeque` body is correct
 * for both, mirroring how [com.hub.media.core.database]'s `DatabaseFileOps.android.kt`/`.jvm.kt`
