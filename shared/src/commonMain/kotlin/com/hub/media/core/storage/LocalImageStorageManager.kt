...
public class LocalImageStorageManager(
    ...
    private val lockRegistry: FileLockRegistry = FileLockRegistry(),
) {
    /**
    ...
    /**
     * Executes [block] while holding a lock for the given [hash].
     ...
     *
     * @param useLock Whether to acquire a per-hash lock before deleting. Defaults to `true`.
     *   Callers already holding a lock for this hash (e.g. [DeleteMediaUseCase]) should pass `false`
     *   to avoid deadlocking the non-reentrant [lockRegistry].
