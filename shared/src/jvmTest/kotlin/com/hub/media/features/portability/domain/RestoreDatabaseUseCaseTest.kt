...
class RestoreDatabaseUseCaseTest {
    ...
    @Test
    fun stage_notASqliteFile_isRejectedAndFileIsDeleted() =
        runTest {
    ...
    @Test
    fun stage_emptyFile_isRejected() =
        runTest {
    ...
    @Test
    fun stage_missingFile_isRejected() =
        runTest {
    ...
    @Test
    fun stage_schemaVersionNewerThanCurrent_isRejectedAndFileIsDeleted() =
        runTest {
    ...
    @Test
    fun stage_currentSchemaVersion_isAccepted_andNotFlaggedAsOlder() =
        runTest {
            ...
            assertTrue(
                File(candidatePath).exists(),
                "an accepted candidate must not be deleted -- it's staged for commit",
            )
        }
     ...
     */
    @Test
    fun sqliteOpenReadOnlyFlag_rejectsWrites_thatTheDefaultOpenFlagsWouldSilentlyAllow() =
        runTest {
     ...
     */
    @Test
    fun stage_truncatedFileWithValidHeader_isRejectedDespitePassingHeaderCheck() =
        runTest {
     ...
     */
    @Test
    fun stage_validSqliteFileThatIsNotAMediaTrackerDatabase_isRejected() =
        runTest {
     ...
     */
    @Test
    fun stage_v4DatabaseMissingAppSettingsTable_isRejectedBeforeCommit() =
        runTest {
    ...
    @Test
    fun stage_olderSchemaVersion_isAcceptedAndFlaggedAsOlder() =
        runTest {
    ...
    @Test
    fun commit_olderSchemaVersion_swapsIn_andNextOpenMigratesForwardWithDataIntact() =
        runTest {
    ...
    @Test
    fun commit_roundTrip_backupThenRestoreElsewhere_dataMatchesFieldForField() =
        runTest {
    ...
    @Test
    fun commit_stagedFileMissing_failsCleanly_andOriginalLibraryIsRestoredAndOpenable() =
        runTest {
            val livePath = path("live.db")
     ...
     */
    @Test
    fun commit_walSidecarRenameFailure_abortsAndRestoresOriginal_neverReportingFalseSuccess() =
        runTest {
            val livePath = path("live.db")
    ...
    @Test
    fun commit_noLiveDatabaseExistedYet_stillSwapsInSuccessfully() =
        runTest {
    ...
    @Test
    fun selfHeal_liveFilePresent_isANoOp() =
        runTest {
            val livePath = path("live.db")
    ...
    @Test
    fun selfHeal_liveFileMissingButBackupExists_restoresFromBackup() =
        runTest {
            val livePath = path("live.db")
            val liveDb = openDatabase(livePath)
            val bookRepository = BookRepository(liveDb)
    ...
    @Test
    fun selfHeal_neitherLiveNorBackupExists_isANoOp() =
        runTest {
            val livePath = path("live.db")
            selfHealDatabaseIfNeeded(livePath)
            assertFalse(File(livePath).exists())
     ...
     */
    @Test
    fun selfHeal_walRenameFailure_neverLeavesLiveFilePresentWithoutIt() =
        runTest {
            val livePath = path("live.db")
     ...
     */
    @Test
    fun stage_notASqliteFile_logsWarningNamingWhyItWasRejected() =
        runTest {
            val candidatePath = path("not-a-db.txt")
            File(candidatePath).writeText("this is definitely not a sqlite database")
    ...
    @Test
    fun stage_schemaVersionNewerThanCurrent_logsWarningWithVersionNumbers() =
        runTest {
            val candidatePath = path("future.db")
            createFreshDatabaseFile(candidatePath)
            BundledSQLiteDriver().open(candidatePath).use { connection ->
     ...
     */
    @Test
    fun stage_notAMediaTrackerDatabase_logsWarningNamingMissingTables() =
        runTest {
            val candidatePath = path("unrelated.db")
            BundledSQLiteDriver().open(candidatePath).use { connection ->
                connection.execSQL("CREATE TABLE some_other_apps_table (id INTEGER PRIMARY KEY, note TEXT)")
     ...
     */
    @Test
    fun commit_walSidecarRenameFailure_logsErrorWithoutEchoingFileContent() =
        runTest {
            val livePath = path("live.db")
            val walPath = "$livePath-wal"
            val shmPath = "$livePath-shm"
