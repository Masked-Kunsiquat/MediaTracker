...
class DatabaseBackupUseCaseTest {
     ...
     */
    @Test
    fun execute_capturesRowStillOnlyInWal_notYetCheckpointedToMainFile() =
        runTest {
            ...
            try {
                val titles =
                    backupDb
                        ...
                        .map { it.title }
                assertEquals(
                    listOf("WAL Safety Check"),
                    titles,
                    "the backup must contain the row that was only in the WAL",
                )
            } finally {
                backupDb.close()
                stagedFile.delete()
    ...
    @Test
    fun execute_emptyDatabase_producesAnOpenableBackup() =
        runTest {
            val liveDb = openLiveDatabase()
            val useCase = DefaultDatabaseBackupUseCase(liveDb, dbFile.absolutePath)
            val result = useCase.execute()
    ...
    @Test
    fun execute_suggestedFileNameHasDbExtensionAndPrefix() =
        runTest {
            val liveDb = openLiveDatabase()
            val useCase = DefaultDatabaseBackupUseCase(liveDb, dbFile.absolutePath)
            val result = useCase.execute()
     ...
     */
    @Test
    fun execute_decoyLogFileBesideLiveDatabase_backupBytesContainNoLogMarker() =
        runTest {
     ...
     */
    @Test
    fun execute_backupSchemaContainsNoLogRelatedTable() =
        runTest {
            val liveDb = openLiveDatabase()
            val bookRepository = BookRepository(liveDb)
            val addResult =
