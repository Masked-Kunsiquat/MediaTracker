...
class ExternalIdentifierDaoTest {
    ...
    @Test
    fun insert_conflictingCompositeKey_replacesExistingRow() = runTest {
    ...
    @Test
    fun observeForMedia_emitsRowsForDifferentProviders() = runTest {
        val media = sampleMediaItem()
        db.mediaItemDao().insert(media)
        ...
        assertEquals(2, db.externalIdentifierDao().observeForMedia(media.id).first().size)
    }

    @Test
    fun cascadeDelete_removesIdentifiersWhenMediaItemDeleted() = runTest {
        val media = sampleMediaItem()
        db.mediaItemDao().insert(media)
        ...
        assertTrue(db.externalIdentifierDao().observeForMedia(media.id).first().isEmpty())
