...
class BookDetailsDaoTest {
    ...
    @Test
    fun insertAndGetByMediaId_returnsInsertedDetails() = runTest {
    ...
    @Test
    fun zeroPageBook_isStoredAndReadBackExactly() = runTest {
        val media = sampleMediaItem()
        db.mediaItemDao().insert(media)
    ...
    @Test
    fun cascadeDelete_removesBookDetailsWhenMediaItemDeleted() = runTest {
        val media = sampleMediaItem()
        db.mediaItemDao().insert(media)
