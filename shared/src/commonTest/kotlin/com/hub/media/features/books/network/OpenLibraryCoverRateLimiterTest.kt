...
class OpenLibraryCoverRateLimiterTest {
    @Test
    fun allowsUpToTheConfiguredMaxWithinTheWindow() =
        runTest {
    ...
    @Test
    fun deniedOutcome_reportsRetryAfterBasedOnTheOldestRequestInTheWindow() =
        runTest {
            val clock = MutableFakeClock(Instant.fromEpochMilliseconds(0))
    ...
    @Test
    fun slidingWindow_expiredRequestsFreeUpSlotsAutomatically() =
        runTest {
            val clock = MutableFakeClock(Instant.fromEpochMilliseconds(0))
            val limiter = OpenLibraryCoverRateLimiter(maxRequestsPerWindow = 1, window = 5.minutes, clock = clock)
    ...
    @Test
    fun recordServerRefusal_deniesEveryTryAcquireUntilItElapses_evenUnderTheLocalCap() =
        runTest {
            val clock = MutableFakeClock(Instant.fromEpochMilliseconds(0))
    ...
    @Test
    fun recordServerRefusal_neverShortensAnExistingLaterRefusal() =
        runTest {
            val clock = MutableFakeClock(Instant.fromEpochMilliseconds(0))
            val limiter = OpenLibraryCoverRateLimiter(maxRequestsPerWindow = 100, window = 5.minutes, clock = clock)
