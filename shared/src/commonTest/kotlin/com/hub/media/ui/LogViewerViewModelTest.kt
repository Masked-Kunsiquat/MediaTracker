...
class LogViewerViewModelTest {
    ...
    @Test
    fun uiState_onFirstOpen_hasNoBoundaryBecauseNothingIsNewYet() = runTest {
        val store = newStore()
        store.log("a", "b")
    ...
    @Test
    fun refresh_withEntriesAddedSinceOpen_marksTheDividerAtTheFirstNewEntry() = runTest {
        val store = newStore()
    ...
    @Test
    fun refresh_calledRepeatedly_movesTheDividerToEachRefreshsOwnNewEntries() = runTest {
        val store = newStore()
        store.log("old")
        val vm = viewModel(store)
        vm.awaitLoaded()
    ...
    @Test
    fun refresh_withNothingNewSinceOpen_marksNothing() = runTest {
        val store = newStore()
        store.log("only")
        val vm = viewModel(store)
        vm.awaitLoaded()
    ...
    @Test
    fun refresh_whenOlderEntriesRotatedOffDiskInBetween_stillMarksTheCorrectEntry() = runTest {
        ...
        val vm = viewModel(store)
        vm.awaitLoaded()
        val boundarySeq = vm.uiState.value.entries.maxOf { it.seq }
    ...
    @Test
    fun readFullLogForExport_returnsEveryRetainedEntryNotJustTheWindow() = runTest {
        val store = newStore()
