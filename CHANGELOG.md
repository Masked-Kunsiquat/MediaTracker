...
- **Tests can now see where a control is, not just that it exists** (#96, Phase A). The keyboard bug fixed above was invisible to the whole suite by construction: the stranded button was present, enabled and "displayed", so every semantics assertion passed while 95 instrumented and 966 unit tests stayed green. What was wrong was its geometry, and nothing here read geometry. A new Robolectric lane in `app/src/test/` reports a keyboard-sized inset to a screen and fails any control whose bounds land underneath it — nodes inside a scrolling container excepted, since those can be scrolled back out. Proved by falsification rather than by passing: with the fix stripped from `LibraryScreen` it fails and names the offender, and it reproduces the same fault the device measurement found. Unlike the existing instrumented suite this lane needs no device, so `:app:testDebugUnitTest` has joined the verification command in AGENTS.md §7 and the required CI job — a job that already exists, because a new one would not be a required check until the branch ruleset was updated to match.
