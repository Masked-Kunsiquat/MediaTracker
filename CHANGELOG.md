...
- **TV shows, tracked per episode (ROADMAP Task 13 Phase C)** — Shows sit in the library alongside books and films, and you tick off individual episodes rather than setting a show-level status.
  ...
  - **Correcting a season's length upwards adds only what is missing.** Going from 10 episodes to 12 adds two, leaving the ten that already exist — and anything you had already watched — untouched.
  - **Known limitation: a season can only grow.** There is no way yet to remove an episode or a season, so quick-filling 20 episodes when you meant 10 leaves ten that cannot be deleted, and the show can never reach Finished. Getting rid of them means deleting the whole show and adding it again, which loses everything watched on it. Worth typing the count carefully until this is fixed.
