...
- **A show can now record which TMDB record it came from** (#75). Adding a show wrote its title, its details and its episode rows, and nothing else — so every show in the library looked identical to every other one, whether it had been typed in by hand or read from a catalogue. There is no add-by-search path yet, but this is the piece it has to have first: without a stored provider id, the backfill that fills in episode titles and air dates has no way to know which show to ask about, and the only way back is the user identifying it a second time.

  The mapping is written in the same transaction as the rest of the show rather than immediately afterwards. The failure that matters here is not a rejected write — it is a process death between two writes, which leaves a show in the library that nothing can trace back to its source, indistinguishable from one somebody typed. That is the row this project keeps having to dig itself out of later with a full crawl over the library; writing it with the show is what stops it existing.

  Listing the same provider twice fails the whole insert and leaves no show behind. Unlike a season number entered twice, this is not pre-checked and given a sentence of its own: a repeated provider cannot come from anything a user types, only from a caller assembling the list wrongly.
