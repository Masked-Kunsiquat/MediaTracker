...
- **Films and shows added by search arrive with their poster.** The artwork is downloaded and kept on the device rather than linked to, so it is still there when you have no signal — the same way book covers already work.
  ...
  If the download fails, or the catalogue has no artwork for that title, nothing else is affected: the title is already in your library by then, and an entry without a picture is a complete one. You are not told about it, because it is not a problem you caused or can act on.

  **Episode stills are deliberately not downloaded.** A single long-running show can hold hundreds of episodes, and fetching an image for every one would cost hundreds of requests and a great deal of storage for pictures that a checklist never shows at any size worth having. The column that would hold them stays empty on purpose.
...
- **A show can now be created with its episodes already titled** (#75). Adding a show could only ever say *how many* episodes a season had — the user types "Season 1: 10 episodes" and ten blank rows appear, which is exactly right for someone working from memory. But a show looked up in a catalogue arrives with every episode's title, air date, length, synopsis and score already in hand, from a single request, and there was nowhere to put any of it.
  ...
  Both forms now sit side by side, and a single show can use either or both — a season typed in by hand and a season that came from a provider are both just seasons. Nothing yet calls the new one; this is the shape add-by-search needs to exist before it can be written.

  Two decisions worth stating. A season's episode list has no length limit, unlike a typed count, which is capped at 500 to catch someone entering `1000000` instead of `10`: an enumerated list cannot be produced by that typo, and capping it would reject the real case of a long-running series catalogued as one continuous season. And a provider that answers with an empty title is recorded as *not knowing* one rather than as knowing an empty one — otherwise the backfill, which only ever fills blanks, would skip it forever.

  Nothing about a newly created episode can claim it has been watched. The watched date is not settable from either form, because a show being added has been watched by nobody and no catalogue is in a position to say otherwise.

- **Films and shows can now record which TMDB record they came from** (#75). Adding either wrote the title, the details and — for a show — its episode rows, and nothing else, so every film and show in the library looked identical to every other one whether it had been typed in by hand or read from a catalogue. There is no add-by-search path yet, but this is the piece it has to have first: without a stored provider id, the backfill that fills in episode titles and air dates has no way to know which show to ask about, and the only way back is the user identifying it a second time. A film's stakes are lower — nothing backfills episode titles onto a film — but the row is still the only thing saying where its title, year and runtime came from, and re-fetching its poster needs it just the same.

  The mapping is written in the same transaction as the rest of the row rather than immediately afterwards. The failure that matters here is not a rejected write — it is a process death between two writes, which leaves something in the library that nothing can trace back to its source, indistinguishable from one somebody typed. That is the row this project keeps having to dig itself out of later with a full crawl over the library; writing it alongside is what stops it existing.

  Listing the same provider twice fails the whole insert and leaves nothing behind. Unlike a season number entered twice, this is not pre-checked and given a sentence of its own: a repeated provider cannot come from anything a user types, only from a caller assembling the list wrongly.

- **A TMDB credential can be entered, tested, and is kept out of backups** (#75). TMDB refuses anonymous requests, so it needs a key the user supplies. It is entered in Settings and stored on the device. It goes to TMDB, because that is what it is for — but nowhere else: the backup scrub removes it before a `.sqlite` snapshot is written, so it is never in a file you export or share, and the restore confirmation warns that you will have to enter it again afterwards. It is also kept out of the application log, including out of the request URLs a log line might otherwise carry.
  ...
  There is a **Test** button once something is saved. Without it, "a credential is saved" is a statement about storage that reads as a statement about working, and the difference would otherwise only surface the first time something actually uses the credential, by which point the cause is several steps behind you. One request now answers it outright. It is offered for TMDB only, because Google Books has no equivalent endpoint and a button that could merely attempt a book lookup would be a vaguer promise wearing the same label.
...
- **The TMDB client reads a show and all its seasons in one request** (#75). Nothing in the app calls this yet — there is no add-by-search here, and no screen looks a film or show up. It is the client the credential above exists for, and this entry records what it does rather than a feature it does not yet provide.

  Fetching a show and then each of its seasons in turn would make a five-season show six round trips. TMDB can fold all of it into a single response carrying the same bytes over one connection, which is a difference felt on a phone rather than on a desk. Seasons that do not exist come back simply absent rather than as an error, so the request does not have to know a show's shape in advance.

  Two things are reported rather than left to be discovered, because both otherwise look exactly like a show that genuinely has fewer seasons: TMDB caps how much one response may carry, and a season whose data cannot be read is dropped on its own instead of failing the whole show.

  The request asks for regular seasons only, never specials. What should create specials, if anything, is open — see #122.
...
- **The instrumented suite is green on a clean checkout again** (#114). One test in `SettingsNavigationTest` had been failing on the device against an untouched `main`, and the cost of that is not the red line itself — it is that the next person to run the suite has to work out that the failure is not theirs, and the person after that stops looking. Nothing was wrong with the app: saving a Google Books API key writes the key, the status line follows it, and clearing it puts the line back. The test was reading the result before it existed.
  ...
  Three things kept that hard to see. The cleanup ran in a `finally` that *asserted*, so whatever actually broke was thrown away in favour of the cleanup's own exception, and the report named a line that was never the problem. That cleanup also went through the screen, which only works when the screen is in the state the test expected — precisely what a failing test cannot promise — so a bad run could leave a bogus credential saved on the device for the next real book lookup to use. And saving and clearing were one test, so a broken save failed the clear half too, since the Clear button is not rendered at all until a key exists.

  Now the key is removed through the repository before and after each test, so a run cannot inherit or bequeath one; saving and clearing are separate tests, so a failure names the callback that broke; and both wait for the status line to actually change rather than assuming it already has.
...
- **The book screen was one 3,255-line file; it is now six, and it looks exactly the same** (#81, #78). The screen behind every book in the library — its details, its reading history, its timer and the dialogs for logging a session — had grown into a single file of 3,255 lines holding 45 separate pieces of interface. Nothing was wrong with it, which is the problem: a file that size is one nobody can hold in their head, and the surest sign of it here was that this was the *only* form in the app that never picked up the shared number-reading helpers every other form adopted. It was too big for that migration to feel safe in.
  ...
  It is now split along the seams a reader would name — the details tab, the reading history, the session dialogs, the formatting helpers, the previews — and the number-reading migration went with it, which closes the last of #78.

  **The move itself changes nothing.** Not one pixel: a reference screenshot was recorded first, in a change carrying nothing else, precisely so that claim could be checked rather than asserted, and it matches byte for byte afterwards. The rest was verified the same way — every comment in the original file accounted for word by word in the new ones, every one of the 63 things it declared still declared, and the only difference in the moved code being the visibility keyword the language requires when a helper and its callers stop sharing a file. Each of the split-out regions was then opened on a real phone, because a screenshot of one tab proves nothing about the other five.

  **The number-reading migration that rode along with it deliberately does change one thing**, described below. The two are separate changes, and only the first is the one being claimed as invisible.

  That one thing: a run of digits long enough to exceed what a number can hold does not *fail* to be read — it quietly becomes infinity — so a field checking only "did that parse" would have accepted it. It is now treated as unreadable, which is what it is. Reaching it requires typing several hundred digits into a position field, so nobody will notice the difference; it is called out because it is a behaviour change and the paragraph above claims the rest is not.

- **The metadata rules that were never about books no longer live in a book-named object** (#81). A title must not be blank, and a price must be a finite, non-negative number — neither depends on what kind of thing is being described. Checking a release year is a third case, and a mixed one: *how* the check works and what it says when it fails are the same everywhere, but the earliest year that makes sense is not. All three were written for books first, and when films and shows arrived both of the new validators simply reached across and called the book one. Both carried the same note in their own documentation admitting this was the wrong shape and saying the fix was being deferred — a deferral written when there was one borrower, and still there when there were four.

  The shared rules now sit in a media-neutral home, and the book, film and show validators are peers of one another that each own only what is genuinely theirs: page counts, runtimes, seasons and episode counts — and their own earliest year, which each passes in to the shared check rather than re-implementing around it. Those floors keep their individual reasoning, because that part really is different: printing predates cinema by four centuries and broadcast television by four and a half, and a floor shared across all three would accept a film released before film existed.

  Nothing a user can see changes: every rejection message is identical, and the checks reject and accept exactly what they did before. The one rule worth not having copied is the price check, whose correctness rests on the detail that `NaN` compares false against every bound, so a re-derived version would quietly accept a price that poisons every total it is later added to.

- **Eighteen near-identical ViewModel factories became eighteen declarations of what differs** (#81). Every screen in the app gets its view-model built by a small factory class that hands it its dependencies, and each of those eighteen classes carried its own copy of the same ten lines: check the requested class is the one this factory makes, build it, cast, and otherwise fail. The only parts that ever varied were which class and which constructor call. Those two are now the only things each factory states, and the shared half is written once. Two hundred and twenty-seven lines removed.

  The factories keep their names and their constructors, so not one call site changes and every cross-reference to them elsewhere still points at a real, still-documented class. That is deliberate: a version of this change that collapsed them into anonymous lambdas would have deleted more lines and touched eighteen screen files to do it.

- **A rule that was enforced by remembering is now enforced by the compiler** (#81). Tests that talk to the database can only run in one of the project's two test lanes; the other has no real Android environment for the database to open against. Which lane a test ran in was decided by a list of nineteen entries kept by hand in the build file, and every author of a new database test had to remember to go and add themselves to it. Three were added during a single phase of work, and an agent had to be told about the requirement explicitly or it would have shipped a broken build.

  Those tests now live in the lane that can run them, and the helper that opens a test database is declared there and nowhere else. Putting a database test in the wrong place no longer produces a test that quietly never runs — it does not compile. The list is deleted.

  The list had already drifted, which is what this kind of rule does. Because it excluded whole packages rather than named tests, it had swept up two tests that touch no database at all and stopped them running on the pull-request gate. One of them described itself, in its own documentation, as running everywhere. It did not, and nothing reported that. Both run now.
...
- **Tests can now see where a control is, not just that it exists** (#96, Phase A). The keyboard bug fixed above was invisible to the whole suite by construction: the stranded button was present, enabled and "displayed", so every semantics assertion passed while 95 instrumented and 966 unit tests stayed green. What was wrong was its geometry, and nothing here read geometry. A new Robolectric lane in `app/src/test/` reports a keyboard-sized inset to a screen and fails any control whose bounds land underneath it. Seven screens are covered: the library, both add-and-edit pairs for books and films, add show, and settings. Controls inside a scrolling list are measured with the list scrolled to its end, since that is the highest a row can travel — covered there means covered at every scroll position. Each test also runs a positive control first, refusing to pass a fixture too sparse or too short to have been able to fail. Every one of the seven was then proved by falsification rather than by passing: strip the fix from that screen, confirm its test fails, restore. That sweep is what caught four of these tests asserting nothing at all in their first draft, which no green run would have shown. Unlike the existing instrumented suite this lane needs no device, so `:app:testDebugUnitTest` has joined the verification command in AGENTS.md §7 and the required CI job — a job that already exists, because a new one would not be a required check until the branch ruleset was updated to match.
...
- **TV shows, tracked per episode (ROADMAP Task 13 Phase C)** — Shows sit in the library alongside books and films, and you tick off individual episodes rather than setting a show-level status.
  ...
  - **A season's length can be corrected in either direction.** Growing it adds only what is missing — 10 episodes to 12 adds two and leaves the ten that exist, and everything watched on them, untouched. Shrinking it removes the episodes above the new count, so quick-filling 20 when you meant 10 is a mistake you can fix rather than one you live with. A whole season can be removed too.
  - **Removing episodes asks first, and says what it costs.** Shrinking a season or removing one tells you how many episodes will go and how many of those are marked watched, because their watched dates go with them. Growing a season changes nothing and does not ask.
...
- **A show you are caught up on no longer claims to be finished.** Watching every episode that exists is not the same as the show being over: a running series filed itself under Finished, told you that you were done with it, and then quietly stopped being finished the moment its next season was added. A show known to be still in production now sits under In progress, where there is genuinely more of it to come. A show that has ended — or was cancelled, which is still an end — stays under Finished. Shows whose status is not known keep behaving exactly as before, which today is all of them; the status arrives with provider metadata.
- **A price typed with a comma was stored a hundred times too large.** On a keyboard that offers `,` as the decimal key, "14,99" in a purchase-price field became `1499` — the comma was discarded rather than read, and the digits that survived parsed cleanly, so the wrong number saved with no error and nothing on screen to notice. Numeric fields now read whichever separator the device's keyboard offers as the decimal point, and treat the other one as a thousands separator and drop it: "1,499.50" and "1.499,50" both mean 1499.50, in the locale that writes them that way. Affected every purchase-price field and the book progress fields.

### Internal

- **A numeric form field is read the same way by a screen and its ViewModel.** The "blank means unknown, unreadable must be refused" rule was hand-rolled at seventeen places across five screens while the ViewModels already had it as a shared helper — two definitions of one rule, each correct only as long as the next person copying it got the `||` right. The screens now call the same helper. No user-visible change: the two definitions did disagree on leading and trailing spaces, but every field involved is digit-filtered, so nothing could reach the disagreement.
  - **The three screens this touched that had no Compose tests now have them** — Add TV show, Edit book, and the show detail screen's season-length dialog, taking the instrumented suite from 78 tests to 95. Includes a guard for the season overflow menu staying reachable at a two-digit episode count, which is the layout bug fixed earlier in this release, and for a season shrink not deleting episodes until it is confirmed.
- **Schema v6 gains the columns TMDB will fill, decided while it is still editable.** v6 has not shipped in a tag, so it can still be corrected in place; after the release the same additions would each cost a v7 and its own migration. Nothing writes any of them yet. `tv_details` gains `airingStatus` (continuing/ended/cancelled), `overview`, `firstAirDate` and `lastAirDate`; `episodes` gains `runtimeMinutes`, `overview`, `stillImageHash` and `communityRating`; `media_items` gains `communityRating`. All nullable, no existing row rewritten.
  - **`airingStatus` is about the show, not the viewer.** "Finished" meant "every episode row is watched", which conflates *up to date* (all aired episodes seen, show still running) with *completed* (all episodes there will ever be). Those are different states and the library showed the same chip for both. `tv_details.status` is the user's own watch status, which is why the new column is not called `status`.
  - **Per-episode runtime, not per-show.** Episode lengths are not uniform — pilots and finales run long, and TMDB reports the show-level figure as an array for that reason — so any "hours watched" total has to sum real episode lengths.
  - **Stills are stored as local content hashes**, matching how covers already work: an offline-first app that kept remote image URLs would render blank the moment the device lost signal.
  - **`communityRating` is the provider's score, not yours.** Kept so "did other people like it" can eventually sit beside your own rating; that one belongs to Task 10, which may attach it to a re-read rather than to the title.
...
- **Unified Media Architecture (Issue #67)** — Foundation for supporting heterogeneous media types (Books, Movies, TV Shows) uniformly across the app. No user-visible effect in this release: nothing yet creates a movie or TV row, so every polymorphic branch is groundwork.
...
- **Heterogeneous Library UI**: The library screen renders mixed media types with type-specific release labels and consolidated bulk actions. Not reachable by a user yet, for the same reason as above.
...
codebase and adds a user-supplied Google Books API key to harden lookup reliability. It also
includes several reliability fixes for settings management and database restoration.
...
- **Transport failures in Google Books lookups no longer risk leaking credentials.** The client now omits the underlying exception from the log on transport-level errors, ensuring an API key embedded in a request URL (by Ktor or a platform engine) never reaches the log file.
- **Settings save/clear actions now accurately report their outcome.** The screen now waits for the database write to complete and only shows the success confirmation when it genuinely succeeds, rather than assuming success.
- **Restore validation is more robust when checking for an existing key.** If reading the current database's API key fails during the restore process, the app now reports a validation error instead of guessing, allowing the user to retry safely.

### Internal
- **Extracted project skills (.skills/)** — Formalized MediaTracker's architectural standards, testing patterns, data portability rules, and release rituals into reusable skills for AI agents. This ensures consistent collaboration across the project's strict local-first and privacy-focused requirements.
- **Instrumented test hardening** — The Settings navigation test now guarantees the API key is cleared even after a failure, preventing side effects on subsequent test runs on the same device.
...
- **CI actions updated, and `gradle/actions` deliberately held at v5 (CI)** — the first Dependabot run proposed four updates; three are taken (`actions/checkout` v7.0.1, `actions/upload-artifact` v7.0.1, `android-actions/setup-android` v4.0.1). `gradle/actions` stays at v5.0.2 rather than v6, because v6 moved its caching into `gradle-actions-caching` — a proprietary component that is no longer MIT-licensed, whose use means accepting Gradle's commercial Terms of Use, and which loads whenever caching is enabled, as it is here. That is a licensing decision rather than a version bump, and it passes CI green either way, so `.github/dependabot.yml` now blocks major updates for that action rather than re-litigating it weekly. The ignore rule must name the full action path (`gradle/actions/setup-gradle`), not the repository — named as the repository it silently matches nothing, and v6 is proposed again the same day.
- **A failing test run uploads its reports again (CI)** — the `test` job had no upload step at all, while the `lint` job carried one naming `shared/build/test-results/`, a path that only exists where the tests ran. It was captured when the lint job was inserted directly above it, so every failing test run since had uploaded nothing and left only log output to diagnose from — the exact situation that step exists to prevent. Both `if: failure()` upload paths are now verified end to end by a deliberately failing run, which is the only way to exercise them: on a green pipeline they never execute, and they had just been carried across three major versions of `actions/upload-artifact` untested.
- **CI is now enforced, not advisory (repo settings)** — the `protect-main` ruleset already required a pull request and blocked force-pushes and deletion of `main`, but nothing required the checks to pass, so a red PR could be merged. The five CI jobs are now required status checks. CodeRabbit and GitGuardian are deliberately excluded: CodeRabbit reports `pass` without having reviewed anything in at least four situations, so as a gate it certifies nothing. Merged branches are also deleted automatically now. AGENTS.md §7 records the hazard that comes with this — required checks match by job name, so renaming one in `ci.yml` silently blocks every pull request.
- **AGENTS.md §5 records the action-pinning convention** — including how to resolve a SHA correctly. Some tags are annotated, so the ref points at a tag object rather than a commit, and the naive lookup works for four of the five actions here and fails on the fifth. Undocumented, the convention would have eroded the first time someone added a job.
