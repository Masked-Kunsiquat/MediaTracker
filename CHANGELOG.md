...
- **The book screen was one 3,255-line file; it is now six, and it looks exactly the same** (#81, #78). The screen behind every book in the library — its details, its reading history, its timer and the dialogs for logging a session — had grown into a single file of 3,255 lines holding 45 separate pieces of interface. Nothing was wrong with it, which is the problem: a file that size is one nobody can hold in their head, and the surest sign of it here was that this was the *only* form in the app that never picked up the shared number-reading helpers every other form adopted. It was too big for that migration to feel safe in.
  ...
  Not one pixel moves. A reference screenshot was recorded first, in a change carrying nothing else, precisely so that claim could be checked rather than asserted; it matches byte for byte afterwards. The rest was verified the same way: every comment in the original file was accounted for word by word in the new ones, every one of the 63 things it declared is still declared, and the only difference in the code itself is the visibility keyword the language requires when a helper and its callers stop sharing a file. Each of the split-out regions was then opened on a real phone, because a screenshot of one tab proves nothing about the other five.

  One genuine improvement came out of the migration rather than the move. A run of digits long enough to exceed what a number can hold does not *fail* to be read — it quietly becomes infinity — so a field checking only "did that parse" would have accepted it. It is now treated as unreadable, which is what it is.
