...
2. **Polymorphic Media Schema**: 
   ...
   - Child tables (e.g., `BookDetails`, `MovieDetails`) linked by `media_id` Foreign Key.
3. **Immutability**: Prefer `val` over `var`. Use data classes with `.copy()` for state updates.
