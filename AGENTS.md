...
* **If a device is attached and the change touches `app/src/`, run the instrumented suite. Not optional, and not a judgement call.** The bullet above says not to treat this lane's absence from the CI gate as permission to skip it; this is what that means in practice, because the soft version did not hold.
  ...
  **While iterating, you may keep the app installed.** Single-class and in-progress runs can add `-Pandroid.injected.androidTest.leaveApksInstalledAfterRun=true`. The suite takes just as long, but the app and its data survive, so there is no 80-second `seedDebugDevice` afterwards. Verified on AGP 9.1.1: both APKs and the seeded library survived a full 160-test run. **The run this rule requires, before opening or updating a PR, omits the flag.** The uninstall is what makes each run start from an empty app, and that is the guard against the next bullet's failure, which nothing else checks because CI does not run this lane.
...
* **Frozen schema ledger.** Every version below shipped in a tag and is therefore immutable. **Append a row here in the same commit that bumps `APP_DATABASE_VERSION`** — this table is the rule's only record, and a version missing from it is a version nobody can tell is frozen.
  ...
  | v7 | `v0.20.0` | `MIGRATION_6_7` — moves `tv_details.overview` to `media_items.synopsis` (#133) |
  ...
  **Every row above is now frozen.** v7 was the most recent to sit unfrozen, and the distinction was the rule rather than an exception to it: the freeze attaches to *shipping in a tag*, not to the version existing. It was held unreleased from PR #76 until Movies and TV actually worked, and that window was spent deliberately rather than allowed to lapse — the release-blocking question (whether TMDB needs a per-episode provider id, which would have meant a new table) was answered *no* against the API docs, and the columns Phase D will fill were added while adding them was still free: `tv_details.airingStatus`/`overview`/`firstAirDate`/`lastAirDate`, `episodes.runtimeMinutes`/`overview`/`stillImageHash`/`communityRating`, and `media_items.communityRating`. See PR #86 and ROADMAP Task 13 Phase D.
